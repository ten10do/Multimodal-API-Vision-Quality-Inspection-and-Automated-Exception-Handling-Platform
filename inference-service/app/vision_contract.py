"""Standardized vision inference contract.

Phase 1C decision: the vision pipeline emits objective detection facts only.
severity, PASS / REVIEW / FAIL and any quality judgement are produced downstream
by the Quality Rule Engine and MUST NOT appear in this layer.
"""
...
# 6 NEU-DET defect classes, index order matches YOLO training data.yaml
...
class Detection(BaseModel):
    ...
    defect_area_ratio: float = Field(
        ge=0.0, le=1.0, description="Bounding box area divided by image area"
    )
